...
struct FlatMainloopTmaWarpSpecializedKdaFwd {
            ...
            if constexpr (!is_first_block) {
                ...
                cutlass::arch::NamedBarrier::arrive_and_wait(NumStateMmaThreads, KdaNamedBarriers::StateMath);
                {
                    ...
                    for (int s = 0; s < 2; ++s) {
                        ...
                        for_each(make_int_sequence<size(tQcMq_quar)>{}, [&](auto i) {
                            int row = int(get<0>(tQcMq_quar(i)));
                            float norm_s = kUseRealNorm ? storage.smem_norm_partial[row][0] : 1.0f;
                        ...
                        for_each(make_int_sequence<size(tQcMq_quar)>{}, [&](auto i) {
                            int row = int(get<0>(tQcMq_quar(i)));
                            float norm_s = kUseRealNorm ? storage.smem_norm_partial[row][1] : 1.0f;
            ...
            // WG1 (thread_idx >= 128): wg_idx=1, alpha_last indices {2,3}, K/output dim1=1
            {
                ...
                for (int s = 0; s < 2; ++s) {
                    ...
                    for_each(make_int_sequence<size(tQcMq_quar)>{}, [&](auto i) {
                        ...
                        float norm_s = kUseRealNorm ? storage.smem_norm_partial[int(seq)][1] : 1.0f;
    ...
    compute_aux_safe(
        ...
        auto qk_kk_subchunk_mma_and_store = [&](int blk) INLINE_LAMBDA {
            ...
            // Hardware TV mapping for BF16 m16n8k16 MMA (from PTX ISA):
            //   Operand A fragment: indices (i%4)<2 → row lane/4, (i%4)>=2 → row lane/4+8
            //   Operand B fragment: all elements → token = warp_in_mma*8 + lane/4
            int lane_id_norm = local_thread_idx % 32;
            int g_norm = lane_id_norm / 4;                 // M-group: 0-7
            int warp_in_mma_norm = local_thread_idx / 32;  // 0 or 1 (2-warp MMA)
            ...
            auto s2r_compute_subchunk_operandA = [&](auto r_, int j, int j0, int j1) INLINE_LAMBDA {
                ...
                cute::transform(tQKrQ_r_j_bf16, tArA_r_j, tQKrQ_r_j_float, [](auto q, auto a) { return float(q) * a; });
                ...
                cute::transform(tQKrK_r_j_bf16, tArA_r_j, tQKrK_r_j_float, [](auto k, auto a) { return float(k) * a; });
            ...
            auto s2r_compute_subchunk_operandB =
                [&](auto c_, int j, int j0, int j1, auto const& tArAfirst_kt) INLINE_LAMBDA {
                    ...
                    cute::transform(
                        tQKrKt_c_j_bf16, tArA_c_j, tQKrKt_c_j_float, [](auto k, auto a) { return float(k) * a; });
                ...
                [&](auto r_, auto c_, auto is_diagonal_, auto is_first_subchunk_) INLINE_LAMBDA {
                    ...
                    if constexpr (is_first_subchunk) {
                        ...
                        if constexpr (kUseRealNorm) {
                            ...
                            for (int c = 0; c < 4; c++) {
                                cached_norm_kt[c] =
                                    storage.smem_norm_partial[c * 16 + warp_in_mma_norm * 8 + g_norm][1];
                            }
                        } else {
                            for (int r = 0; r < 4; r++) {
                                cached_norm_q_lo[r] = cached_norm_q_hi[r] = 1.0f;
                                cached_norm_k_lo[r] = cached_norm_k_hi[r] = 1.0f;
                            }
                            for (int c = 0; c < 4; c++) {
                                cached_norm_kt[c] = 1.0f;
                            }
                        }
                    }
